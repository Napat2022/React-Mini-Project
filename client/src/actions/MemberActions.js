...
import { MEMBER_CREATE, MEMBERS_FETCH, MEMBER_UPDATE, MEMBER_FETCH } from "./types";
...
export const membersFetch =() => {
    return dispatch => {
        axios.get(process.env.REACT_APP_API_URL + "/members").then (res => {
...
export const memberCreate = values => {
    return dispatch => {
        axios.post(process.env.REACT_APP_API_URL + "/members", values).then ( res=>{
...
export const memberUpdate = (id, values) => {
    return dispatch => {
        axios.put(process.env.REACT_APP_API_URL + "/members/" + id, values).then ( res=>{
